...
// Fetch bill details from the 'bill' table based on bill_id
if (isset($_GET['bill_id'])) {
    $bill_id = $_GET['bill_id'];

    // Fetch bill details
    $sqlBillDetails = "SELECT * FROM bill WHERE bill_id = '$bill_id'";
    ...
    if ($resultBillDetails && mysqli_num_rows($resultBillDetails) > 0) {
        ...
        // Fetch customer name from the 'customer_info' table
        $customer_id = $billDetails['customer_id'];
        $sqlCustomerName = "SELECT name FROM customer_info WHERE id = '$customer_id'";
        ...
        if ($resultCustomerName && mysqli_num_rows($resultCustomerName) > 0) {
            ...
            if ($resultOrderDetails && mysqli_num_rows($resultOrderDetails) > 0) {
                ...
                // Display customer name, total amount, and bill date
                echo "<table border='1'>";
        ...
        echo "<p>Error fetching bill details.</p>";
    }
} else {
